...
//        viewModel = ViewModelProviders.of(MyApplication.getMyApplication()).get(UserViewModel.class);
        locationViewModel=new LocationViewModel(MyApplication.getMyApplication());

        MySharedPreferences sp=MySharedPreferences.getInstance(getApplicationContext());
        if(sp.chk("switch") && sp.get("switch").equalsIgnoreCase("1")){
            locationViewModel.addloc(lat,lng);
